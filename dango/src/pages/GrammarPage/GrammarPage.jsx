...
import GrammarPageForm from "../../component/Forms/GrammarPageForm/GrammarPageForm";
import axios from "axios";

const GrammarPage = () => {
  // grammarData는 함수의 시작 부분에서 정의
  const grammarData = [
    {
      hanja: "日本",
      reading: "にほん",
      ...
      example1: "これは日本です。",
      example2:
        "日本に行きたい日本に行きたいです日本に行きたいです日本に行きたいですです。" +
        "日本に行きたい日本に行きたいです日本に行きたいです日本に行きたいですです" +
        "日本に行きたい日本に行きたいです日本に行きたいです日本に行きたいですです",
    },
    {
  ...
  const fetchAndStoreData = async () => {
    try {
      ...
      const processedData = response.data.map((item) => ({
        ...
        exampleTranslation1: item.exampleTranslation1,
        example2: item.example2,
        exampleTranslation2: item.exampleTranslation2,
      }));
      ...
      localStorage.setItem("grammerDataTimestamp", currentTime); // 타임스탬프 저장
      localStorage.setItem("currentGrammerIndex", 0); // 초기화 시 currentIndex를 0으로
  ...
  const fetchAnotherData = async () => {
    try {
      ...
      const existingData =
        JSON.parse(localStorage.getItem("studyGrammerContent")) || [];

      // 기존 데이터에 새 데이터를 이어 붙이기
      const updatedData = [...existingData, ...processedData];

      // 합쳐진 데이터를 로컬 스토리지에 저장
      localStorage.setItem("studyGrammerContent", JSON.stringify(updatedData));
  ...
  const loadDataWithExpirationCheck = () => {
    ...
    if (storedTimestamp) {
      ...
      if (timeDifference > oneDayInMillis) {
        ...
        localStorage.removeItem("studyGrammerContent");
        localStorage.removeItem("dataTimestamp");
        localStorage.removeItem("currentGrammerIndex");
        return null; // 데이터 만료
      }
    }
  ...
  useEffect(() => {
    ...
    const index =
      parseInt(localStorage.getItem("currentGrammerIndex"), 10) || 0;
  ...
  return (
    <div>
      {/* GrammarPageForm에 데이터를 전달 */}
      <GrammarPageForm
        propDay={formattedDay}
        propGrammarData={studyContentList} // grammarData를 prop으로 전달
      />
    </div>
  );
};
